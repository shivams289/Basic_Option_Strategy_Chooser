...
class Strategy:
    def __init__(self, LS, spot, HS, hsp, lsp):
        ...
        self.premium2 = lsp
        self.market_expiry = [
            expiry for expiry in range(self.spot - 1500, self.spot + 1500, 100)
        ]
        deviation = [(expiry - spot) / spot * 100 for expiry in self.market_expiry]
        self.lsdf = pd.DataFrame(
            {"market_expiry": self.market_expiry, "mx_%deviation": deviation}
        )
    ...
    def long_strangle(self):  # lstrg
        ...
        pe_payoff = [
            (max(self.LS - expiry, 0) - self.premium2) for expiry in self.market_expiry
        ]
    ...
    def short_strangle(self):  # sstrg
        ...
        pe_payoff = [
            (self.premium2 - max(self.LS - expiry, 0)) for expiry in self.market_expiry
        ]
        self.lsdf["ce_payoff"] = ce_payoff
        self.lsdf["pe_payoff"] = pe_payoff
        self.lsdf["strategy_payoff"] = self.lsdf.ce_payoff + self.lsdf.pe_payoff
    ...
    def bull_call_spread(self):  # bcs
        ...
        ls_pe_payoff = [
            (max(expiry - self.LS, 0) - self.premium2) for expiry in self.market_expiry
        ]
        self.lsdf["otm_pe_payoff"] = ls_pe_payoff
        self.lsdf["itm_pe_payoff"] = hs_pe_payoff
        self.lsdf["strategy_payoff"] = self.lsdf.otm_pe_payoff + self.lsdf.itm_pe_payoff
    ...
    def bear_call_spread(self):  # becs
        ...
        ls_ce_payoff = [
            (self.premium2 - max(expiry - self.LS, 0)) for expiry in self.market_expiry
        ]
    ...
    def put_ratio_back_spread(self):  # prbs
        ...
        ls_pe_payoff = [
            (2 * max(self.LS - expiry, 0) - self.premium2)
            for expiry in self.market_expiry
        ]
        self.lsdf["otm_pe_payoff"] = ls_pe_payoff
        self.lsdf["itm_pe_payoff"] = hs_pe_payoff
